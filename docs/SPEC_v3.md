...
#### 术语口径（避免混淆）
本产品区分三个不同维度，勿混为一谈：
- **platform（接入平台）**：从哪个聚合网关调到的数据。取值 openrouter / anyint / zenmux。**对应 snapshots.source 字段**（字段名为 source 是历史命名，语义即 platform）。
- **provider（模型归属）**：模型是哪家厂商的。对应 models.brand 字段（如 Baidu / Z.ai / MiniMax）。
- **channel（通道）**：仅 OpenRouter 内部有，free / standard 两条接入路径。对应 snapshots.is_free 字段。

注意：source='anyint' 表示「经 AnyInt 平台调到」，不代表 AnyInt 是该模型的 provider —— ernie-5.1 的 provider 是 Baidu（经 qianfan 路由），AnyInt 只是接入平台。AnyInt/ZenMux 不分 free/standard 通道，其数据 is_free=false 仅表示「总量/非免费口径」，非真有免费通道。
